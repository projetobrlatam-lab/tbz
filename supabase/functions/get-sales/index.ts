...
serve(async (req: Request) => {
  ...
  try {
    ...
    let produto: string | null = null // Removido o valor padrão
    let traffic_source: string | null = null; // Adicionado traffic_source
    ...
    if (req.method === 'GET') {
      ...
      traffic_source = url.searchParams.get('traffic_source') // Adicionado
    } else {
      const body = await req.json()
      date_filter = body?.date_filter ?? null
      custom_date = body?.custom_date ?? null
      produto = body?.produto ?? null
      traffic_source = body?.traffic_source ?? null // Adicionado
    }
    
    const effectiveDateFilter = date_filter || 'all'
    // Validação: produto é obrigatório se não for 'all'
    if (!produto || (produto === 'all' && date_filter !== 'all')) { // Se o filtro de data não for 'all', o produto deve ser especificado
      // Ou se o produto for 'all' mas não houver filtro de data, ainda é 'all'
    ...
    // Não há coluna traffic_source diretamente em oreino360-compras, então não podemos filtrar aqui.
    // Se necessário, isso exigiria um JOIN com oreino360-leads e oreino360-lead_traffic_details.
