...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (hotmartPayload.event === 'PURCHASE_APPROVED' || hotmartPayload.event === 'PURCHASE_COMPLETE') {
      ...
      let produto_slug: string | null = null; // Adicionado para capturar o slug do produto
      ...
      try {
        ...
        produto_slug = hotmartPayload.data?.product?.id || null; // Tentativa de pegar o ID do produto como slug
      ...
      const { data, error } = await supabaseAdmin
        ...
        .insert({
          ...
          produto: produto_slug, // Adicionado o slug do produto
        })
        .select();
