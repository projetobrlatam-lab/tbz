...
serve(async (req: Request) => {
  ...
  try {
    ...
    // Join com sessoes para pegar fonte_de_trafego e tipo_de_funil
    let query = supabase
      .schema('tbz')
      .from('abandono')
      .select('id, step_where_abandoned, reason, created_at, produto, sessoes!inner(fonte_de_trafego, tipo_de_funil, traffic_id)')
    ...
    if (fonteDeTrafego && fonteDeTrafego !== 'all') {
      query = query.eq('sessoes.fonte_de_trafego', fonteDeTrafego);
    }

    if (tipoDeFunil && tipoDeFunil !== 'all') {
      query = query.eq('sessoes.tipo_de_funil', tipoDeFunil);
    }
    ...
    const transformedData = data?.map((item: any) => ({
      ...
      tempo_gasto: 0, // Não temos mais time_spent_minutes na tabela nova por enquanto
      fonte_trafego: item.sessoes?.fonte_de_trafego || 'Não especificado',
      tipo_funil: item.sessoes?.tipo_de_funil || 'Não especificado',
      traffic_id: item.sessoes?.traffic_id || 'Não especificado'
    })) || [];
