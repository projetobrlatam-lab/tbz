...
const DashboardScreen: React.FC<DashboardScreenProps> = () => {
  ...
  useEffect(() => {
    const checkAuth = async () => {
      try {
        ...
        console.log('[DashboardScreen] Authentication status:', !!session);
      } catch (error) {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange((_event, session) => {
      setSession(session);
      setIsAuthenticated(!!session);
      console.log('[DashboardScreen] Auth state changed:', !!session);
    });
  ...
  useEffect(() => {
    if (!isAuthenticated || !session) {
      console.log('[DashboardScreen] Skipping realtime setup - not authenticated');
      return;
    }
    ...
    const setupRealtime = () => {
      channel = supabase
        ...
        .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'oreino360-eventos' }, (payload) => {
          console.log('🔥 Novo evento detectado (oreino360-eventos):', payload);
          fetchInitialData();
        })
        .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'oreino360-identificador' }, (payload) => {
          console.log('🔥 Nova visita detectada (oreino360-identificador):', payload);
          fetchInitialData();
        })
        .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'oreino360-abandono' }, (payload) => {
          console.log('🔥 Novo abandono detectado (oreino360-abandono):', payload);
          fetchInitialData();
        })
        .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'oreino360-compras' }, (payload) => {
          console.log('🔥 Nova venda detectada (oreino360-compras):', payload);
          fetchInitialData();
        })
        .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'oreino360-leads' }, (payload) => {
          console.log('🔥 Novo lead detectado (oreino360-leads):', payload);
          fetchInitialData();
        })
        .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'oreino360-lead_tag_assignments' }, (payload) => {
          console.log('🔥 Nova atribuição de tag detectada (oreino360-lead_tag_assignments):', payload);
          fetchInitialData();
        })
        .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'oreino360-sessoes' }, (payload) => {
          console.log('🔥 Nova sessão detectada (oreino360-sessoes):', payload);
          fetchInitialData();
        })
        .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'oreino360-leads' }, (payload) => {
          console.log('🔥 Lead atualizado (oreino360-leads):', payload);
          fetchInitialData();
        })
        .on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'oreino360-sessoes' }, (payload) => {
          console.log('🔥 Sessão atualizada (oreino360-sessoes):', payload);
          fetchInitialData();
        })
        .subscribe((status, err) => {
          console.log('🔄 Realtime status changed:', status);
          if (status === 'SUBSCRIBED') {
            console.log('✅ Realtime Dashboard Conectado!');
            console.log('📡 Escutando mudanças em: eventos, identificador, abandono, compras, leads, tag_assignments, sessões');
          }
          if (status === 'CHANNEL_ERROR') {
            console.error('❌ Erro no Realtime Dashboard:', err);
            console.error('❌ Authentication status:', isAuthenticated);
            console.error('❌ Session:', session);
            console.error('❌ Error details:', JSON.stringify(err, null, 2));
          }
          if (status === 'CLOSED') console.log('🔌 Conexão Realtime fechada');
          if (status === 'TIMED_OUT') console.log('⏰ Timeout na conexão Realtime');
        });
    };
    ...
    return () => {
      if (channel) {
        console.log('🔌 Desconectando Realtime Dashboard...');
        channel.unsubscribe();
        channel = null;
      }
    };
  }, [isAuthenticated, fetchInitialData]); // Removendo session das dependências para evitar reconexões desnecessárias
  ...
  const handleLogout = async () => {
    ...
    // Removido window.location.reload() para evitar refresh da página
  ...
  const abandonmentChartData = Object.entries(abandonment_by_step || {})
    .map(([step, stats]: [string, any]) => ({
      label: translateStepName(step),
  ...
  return (
    <div className="min-h-screen bg-gray-50">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center py-6">
            <div className="flex items-center space-x-4">
              ...
              <div>
                {/* <h1 className="text-3xl font-bold text-gray-900">Dashboard Reino 360</h1> */}
                {/* <p className="text-gray-600">Acompanhe o desempenho do seu quiz em tempo real</p> */}
              </div>
            </div>
            <div className="flex items-center space-x-4">
              {/* Filtro de Produto */}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {loading ? ( // Simplificado o loading condicional
